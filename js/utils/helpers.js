// ================================================================
// RaktaSetu — Helper Utilities
// Formatters, random generators, distance calculations
// ================================================================
...
const Helpers = {
  ...
  // Estimate travel time (hours) based on distance
  estimateTravelTime(distanceKm) {
    // Average ambulance speed in Indian cities: 30-60 km/h
    const speed = distanceKm > 50 ? 55 : distanceKm > 20 ? 40 : 30;
  ...
  // Get stock status based on utilization
  getStockStatus(current, capacity) {
    const ratio = current / capacity;
    if (ratio >= 0.6) return { status: 'healthy', color: 'var(--color-success-500)', label: 'Healthy' };
    if (ratio >= 0.3) return { status: 'moderate', color: 'var(--color-warning-500)', label: 'Moderate' };
    return { status: 'critical', color: 'var(--color-accent-600)', label: 'Critical' };
  },

  // Get urgency label
  getUrgencyLabel(score) {
    if (score >= 80) return { label: 'CRITICAL', class: 'badge-critical', color: '#c62828' };
    if (score >= 60) return { label: 'HIGH', class: 'badge-accent', color: '#e53935' };
    if (score >= 40) return { label: 'MEDIUM', class: 'badge-warning', color: '#f9a825' };
    return { label: 'LOW', class: 'badge-success', color: '#00897b' };
  },
  ...
  icon(name) {
    const icons = {
      ...
      agents: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83"/></svg>',
      ...
      prediction: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>',
      emergency: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="7.86 2 16.14 2 22 7.86 22 16.14 16.14 22 7.86 22 2 16.14 2 7.86 7.86 2"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>',
      inventory: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>',
      // KPI icons
      blood: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2.69l5.66 5.66a8 8 0 1 1-11.31 0z"/></svg>',
